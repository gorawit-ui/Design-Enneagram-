...
| 19–20 | 2 | **A/T challenge ทั้งสองข้อ — ล็อกไว้เสมอ** |
| 21 | 1 | MBTI dimension challenge ของแกนที่ margin ต่ำสุด (ไม่รวม A/T) |
| 22–23 | 2 | Core challenge ของ core อันดับ 1 และอันดับ 2 |
| 24 | 1 | Wing challenge ของ core อันดับ 1 |

**แก้ไข 2026-09-10 — ช่อง 19–24 ต่างจากที่ spec เขียนไว้เดิม** ("MBTI Adaptive 2 + Core Challenge 2 +
Wing Challenge 2") เพราะการอ่านตามตัวอักษรใช้จริงไม่ได้ 2 ข้อ:

1. **A/T ไม่มีข้อ foundation เลย** (8 ข้อ = 4 แกน × 2 ไม่รวม A/T) และช่อง adaptive เดิมก็ไม่ได้กันที่ให้
   → ผู้ตอบอาจได้ตัวอักษร A หรือ T ติดไปกับผลโดยไม่เคยถูกถามเรื่องนี้เลย
2. **ต้องล็อก 2 ช่อง ไม่ใช่ 1** เพราะ `scoreAssessment` ถือว่าแกนที่มีคำตอบน้อยกว่า 2 ข้อเป็น
   `ambiguous` → ถ้ากันช่องเดียว `mbti.type` จะเป็น `null` สำหรับทุกคนที่ทำจนจบ ไม่มีข้อยกเว้น
   จึงต้องเขียนข้อ A/T ขึ้นใหม่อีก 1 ข้อ (`c-at2`)

ที่เหลือ 4 ช่องให้ฝั่ง Enneagram 3 ช่อง เพราะเป็นฝั่งที่หลักฐานบางกว่า — แยก 9 core แล้วต่อด้วย wing
ขณะที่ 4 แกน MBTI มีข้อ foundation แกนละ 2 อยู่แล้ว
...
**สมดุล keyed direction — ทำแล้ว 2026-09-10 และเป็นค่าที่คำนวณมา ไม่ใช่เลือกเอง**

เดิมทุกข้อวางตัวเลือกเรียงจากขั้วซ้ายไปขั้วขวาเหมือนกันหมด วัดผลจริงได้ว่า ตอบตัวเลือกที่ 1 รวดทั้ง 24 ข้อ
ให้ผล **ISTJ-A ที่ความมั่นใจระดับ "clear"** และตอบตัวเลือกที่ 4 รวดให้ **ENFP-T "clear"** — คือผลที่
"มั่นใจ" จากคำตอบที่ไม่มีข้อมูลอะไรเลย และรูปแบบนี้เรียนรู้ได้หลังตอบไป 3–4 ข้อ

ชุดข้อที่สลับลำดับตัวเลือกอยู่ใน `REVERSED_ITEMS` (`app/lib/assessment-data.ts`) และ **ได้มาจากการค้นหา**
ด้วย `npm run keying` ซึ่งไล่ทุกชุดที่สลับ 1 ข้อต่อแกนของ MBTI คู่กับทุกชุดย่อยประมาณครึ่งหนึ่งของบล็อก
Enneagram แล้วให้คะแนนว่าการตอบรวดยังให้ผลที่มั่นใจอยู่หรือไม่ ชุดที่ใช้ได้คะแนน 0 — ตอบรวดทุกตำแหน่ง
(1, 2, 3, 4) ได้ `ambiguous` ทั้ง MBTI และ Enneagram

เลือกด้วยมือไม่ได้ผล: ฝั่ง MBTI สมดุลง่ายเพราะมี 2 ขั้วต่อข้อ แต่ข้อ Enneagram แต่ละข้อมี 4 core ต่างกัน
บวกน้ำหนักรอง ชุดที่ทำให้ตำแหน่งหนึ่งแบนจะทำให้อีกตำแหน่งเอียง
