...
import { LARGE_PRINT_CLASS, readLargePrint, writeLargePrint } from "./lib/large-print";
...
export default function Home() {
  ...
  // Read after mount, not during render: the server has no localStorage, and reading it in the
  // initial state would make the first client render disagree with the markup it hydrates.
  const [largePrint, setLargePrint] = useState(false);
  useEffect(() => {
    const stored = readLargePrint();
    setLargePrint(stored);
    document.documentElement.classList.toggle(LARGE_PRINT_CLASS, stored);
  }, []);
